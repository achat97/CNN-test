...
MAX_FLATTEN_DIM = 200000   # high cap: allow frequency as fine as time
...
def to_model_cfg(sampled):
    ...
    kf, kt = sampled["cnn_kernel_freq"], sampled["cnn_kernel_time"]

    cfg = dict(CONFIG)
    cfg.update({
        "cnn_channels":     channels,
        "cnn_kernel":       (kf, kt),
        "cnn_padding":      (kf // 2, kt // 2),   # per-axis "same": time preserved (stride 1), frequency by stride
        "cnn_stride":       strides,
...
def search_space():
    ...
    return {
        ...
        "cnn_kernel_freq":   tune.choice([3, 5, 7]),
        "cnn_kernel_time":   tune.choice([3, 5]),
        "cnn_freq_downsample": tune.choice([2, 3, 4, 5, 6]),  # blocks that halve frequency; lower = finer
...
def train_trainable(sampled, data=None, epochs=25, seed=0):
    ...
    print(f"config: depth={sampled['cnn_depth']} base_ch={sampled['cnn_base_channels']} "
          f"kf={sampled['cnn_kernel_freq']} kt={sampled['cnn_kernel_time']} freq_down={sampled['cnn_freq_downsample']} "
