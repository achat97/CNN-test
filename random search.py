...
def sample_config(rng):
    ...
    return {
        ...
        "cnn_kernel_freq":   rng.choice([3, 5, 7]),
        "cnn_kernel_time":   rng.choice([3, 5]),
        "cnn_freq_downsample": rng.choice([2, 3, 4, 5, 6]),
...
def to_model_cfg(s):
    ...
    kf, kt = s["cnn_kernel_freq"], s["cnn_kernel_time"]
...
def main():
    ...
    for i in range(args.num_samples):
        ...
        print(f"\n[{i + 1}/{args.num_samples}] "
              f"depth={s['cnn_depth']} base_ch={s['cnn_base_channels']} kf={s['cnn_kernel_freq']} kt={s['cnn_kernel_time']} "
